...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  reload: { configPrefixes: ["channels.zalo_fixed"] },
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      return {
        ...
        approveHint: formatPairingApproveHint("zalo_fixed"),
    ...
    collectWarnings: ({ account, cfg }) => {
      ...
      const { groupPolicy } = resolveOpenProviderRuntimeGroupPolicy({
        providerConfigPresent: cfg.channels?.zalo_fixed !== undefined,
      ...
      if (effectiveAllowFrom.length > 0) {
        return [
          `- Zalo groups: groupPolicy="open" allows any member to trigger (mention-gated). Set channels.zalo_fixed.groupPolicy="allowlist" + channels.zalo_fixed.groupAllowFrom to restrict senders.`,
        ];
      }
      return [
        `- Zalo groups: groupPolicy="open" with no groupAllowFrom/allowFrom allowlist; any member can trigger (mention-gated). Set channels.zalo_fixed.groupPolicy="allowlist" + channels.zalo_fixed.groupAllowFrom.`,
      ];
    },
  },
  groups: {
      ...
      return {
        ...
        channels: {
          ...
          zalo_fixed: {
            ...
            enabled: true,
            accounts: {
              ...next.channels?.zalo_fixed?.accounts,
              [accountId]: {
                ...next.channels?.zalo_fixed?.accounts?.[accountId],
                enabled: true,
                ...(input.tokenFile
                  ? { tokenFile: input.tokenFile }
                  : input.token
