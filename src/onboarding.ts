...
function setZaloDmPolicy(
  ...
  const allowFrom =
    dmPolicy === "open" ? addWildcardAllowFrom(cfg.channels?.zalo_fixed?.allowFrom) : undefined;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
  ...
  return {
    ...
    channels: {
      ...
      zalo: {
        ...
        enabled: true,
        accounts: {
          ...cfg.channels?.zalo_fixed?.accounts,
          [accountId]: {
            ...cfg.channels?.zalo_fixed?.accounts?.[accountId],
            enabled: cfg.channels?.zalo_fixed?.accounts?.[accountId]?.enabled ?? true,
            dmPolicy: "allowlist",
            allowFrom: unique,
          },
        },
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  label: "Zalo",
  channel,
  policyKey: "channels.zalo_fixed.dmPolicy",
  allowFromKey: "channels.zalo_fixed.allowFrom",
  getCurrent: (cfg) => (cfg.channels?.zalo_fixed?.dmPolicy ?? "pairing") as "pairing",
          ...
          channels: {
            ...
            zalo: {
              ...
              accounts: {
                ...
                [zaloAccountId]: {
                  ...next.channels?.zalo_fixed?.accounts?.[zaloAccountId],
                  enabled: true,
                  botToken: token,
                },
              },
