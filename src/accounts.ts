...
function listConfiguredAccountIds(cfg: OpenClawConfig): string[] {
  const accounts = (cfg.channels?.zalo_fixed as ZaloConfig | undefined)?.accounts;
...
export function resolveDefaultZaloAccountId(cfg: OpenClawConfig): string {
  const zaloConfig = cfg.channels?.zalo_fixed as ZaloConfig | undefined;
...
function mergeZaloAccountConfig(cfg: OpenClawConfig, accountId: string): ZaloAccountConfig {
  const raw = (cfg.channels?.zalo_fixed ?? {}) as ZaloConfig;
...
export function resolveZaloAccount(params: {
  ...
  const baseEnabled = (params.cfg.channels?.zalo_fixed as ZaloConfig | undefined)?.enabled !== false;
  const merged = mergeZaloAccountConfig(params.cfg, accountId);
  const accountEnabled = merged.enabled !== false;
  const enabled = baseEnabled && accountEnabled;
  const tokenResolution = resolveZaloToken(
    params.cfg.channels?.zalo_fixed as ZaloConfig | undefined,
    accountId,
  );
